...
def upgrade() -> None:
    ...
    op.execute(text(f"UPDATE project_has_associated_user SET role_id = CASE WHEN role = 'DATA_ACCESS' THEN {roles_dict['DATA_ACCESS']} WHEN role = 'METADATA_ACCESS' THEN {roles_dict['METADATA_ACCESS']} ELSE {roles_dict['METADATA_ACCESS']} END"))
...
def downgrade() -> None:
    ...
    op.execute(text(f"UPDATE project_has_associated_user SET role = CASE WHEN role_id  = {roles_dict['DATA_ACCESS']} THEN 'DATA_ACCESS' WHEN role_id  = {roles_dict['METADATA_ACCESS']} THEN 'METADATA_ACCESS' ELSE 'METADATA_ACCESS' END"))
